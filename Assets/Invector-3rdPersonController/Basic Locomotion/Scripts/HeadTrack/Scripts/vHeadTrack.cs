...
namespace Invector.vCharacterController
{
    public class vHeadTrack : MonoBehaviour
    {
        ...
        public Transform head, spine1, spine2, neck;
        ...
        void Start()
        {
            ...
            if (neck == null) Debug.LogWarning("Invector Headtrack Warning - Check your Character Avatar to see if a Neck Bone was assign correctly.");
        ...
        public virtual void SetLookAtPosition(Vector3 point, float strafeHeadWeight, float spineWeight)
        {
            ...
            var _rotX = Quaternion.AngleAxis(_xAngle + offsetHead.x, head.InverseTransformDirection(transform.right));
            var _rotY = Quaternion.AngleAxis(_yAngle + offsetHead.y, head.InverseTransformDirection(transform.up));
        ...
        Vector3 GetLookPoint()
        {
            ...
            if (lookConditions && !IgnoreHeadTrack())
            {
                ...
                if (useLimitAngle && (lastLookTarget ==null || !lastLookTarget.ignoreHeadTrackAngle))
                {
